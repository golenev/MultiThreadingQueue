...
public class SomeDataProvider implements ArgumentsProvider, AfterEachCallback, TestWatcher {
    ...
    public Stream<? extends Arguments> provideArguments(ExtensionContext extensionContext) {
        ...
        argumentsList.forEach(arg -> {
            ...
            extensionContext.getStore(NAMESPACE).put(storeKey, randomName);// кладём результат датапровайдера + соответствующий ему id
            extensionContext.getStore(NAMESPACE).put("qeueu", keysQueue); //каждый id также был положен в потокобезопасную очередь и эта очередь тут кладётся в стору
